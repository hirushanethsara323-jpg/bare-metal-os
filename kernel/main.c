 ...
 * PIT system timer, Serial UART COM1, Real-Time Clock, and Kernel Heap.
 */
...
void run_kernel_shell(void) {
    ...
    terminal_writestring("[OK] Interactive Nothing OS Kernel Shell v0.2.0 Active.\n");
    terminal_writestring("Type 'help' to view available system commands.\n\n");
    ...
    while (1) {
        ...
        if (strcmp(input_buf, "help") == 0 || strcmp(input_buf, "?") == 0) {
            ...
            terminal_writestring("  version     - Print Nothing OS release information\n");
            ...
            terminal_writestring("  klog <msg>  - Send debug log packet to COM1 Serial Port\n");
            terminal_writestring("  sysinfo     - View kernel, memory, driver & IDT architecture\n");
            terminal_writestring("  agents      - View executive AI OS Development Board\n");
            terminal_writestring("  clear       - Clear screen\n");
            terminal_writestring("  reboot      - Hard reset system via keyboard controller\n");
        } else if (strcmp(input_buf, "version") == 0) {
            terminal_setcolor(VGA_COLOR_WHITE);
            terminal_writestring(KERNEL_NAME);
            terminal_writestring(" v");
            terminal_writestring(KERNEL_VERSION);
            terminal_writestring(" (i386 Protected Mode - Enterprise Edition)\nMaintainer: ");
            ...
            terminal_setcolor(VGA_COLOR_LIGHT_GREY);
            terminal_writestring("  Kernel:     Nothing OS v0.2.0\n");
            ...
            terminal_write_hex((uint32_t)&idt_init);
            terminal_writestring("\n  PIC:        8259 Remapped (Master 0x20, Slave 0x28)\n");
            ...
            terminal_writestring("  Input:      PS/2 Keyboard Controller (Port 0x60/0x64)\n");
...
void _kernel_main(void) {
    ...
    terminal_writestring("[OK] ");
    terminal_setcolor(VGA_COLOR_LIGHT_GREY);
    terminal_writestring("VGA Console Engine initialized\n");
